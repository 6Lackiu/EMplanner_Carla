...
def motion_planning(conn):
    while 1:
        ...
        if len(possible_static_obs_) != 0 and possible_static_obs_[0][-1] <= 30:
            ...
            # 计算障碍物的s,l
            obs_s_list, obs_l_list = planner_utils.cal_s_l_fun(static_obs_xy, local_frenet_path_opt_, s_map)
        else:
        ...
        l_list, _, _, _, l_ds_list, _, l_dds_list = \
            planner_utiles.cal_s_l_deri_fun(xy_list=[pred_loc_],
        ...
        l_min, l_max = \
            motion_plan_path_planning.cal_lmin_lmax(dp_path_s=dp_path_s, dp_path_l=dp_path_l,
        ...
        qp_path_l, qp_path_dl, qp_path_ddl = \
            motion_plan_path_planning.Quadratic_planning(l_min, l_max,
                                                         plan_start_l=l_list[0],
                                                         plan_start_dl=l_ds_list[0],
                                                         plan_start_ddl=l_dds_list[0])
        # print(qp_path_l)
        ...
        current_local_frenet_path_opt = \
            motion_plan_path_planning.frenet_2_x_y_theta_kappa(plan_start_s=begin_s_list[0],
...
if __name__ == '__main__':
    ...
    obs_spawn_point1 = All_spawn_points[48]
    obs_actor = world.spawn_actor(obs_vehicle_bp1, obs_spawn_point1)  # type: carla.Vehicle

    # # 静止车辆
    obs_vehicle_bp2 = world.get_blueprint_library().find('vehicle.tesla.model3')
    obs_vehicle_bp2.set_attribute('color', '0,255,0')
    obs_spawn_point2 = All_spawn_points[143]
    # obs_spawn_point2.rotation = model3_spawn_point.rotation + carla.Rotation(row=-90)
    ...
    while True:
        ...
        if count % plan_count == 0:  # 这里表示控制器执行一千规划器执行一次
            ...
            for obs_v, dis in possible_static_obs:
                ...
                print("*****static obstacle*****", obs_v.type_id, dis)
            dynamic_obs_info = []
            for _obs_v, _dis, _speed in possible_dynamic_obs:
                ...
                print("****dynamic obstacle******", _obs_v.type_id, _dis, _speed)
