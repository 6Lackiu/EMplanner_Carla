...
Controller = Vehicle_control(ego_vehicle=model3_actor, vehicle_para=vehicle_para,
                             pathway=local_frenet_path_opt)  # 实例化控制器
...
while True:
    ...
    if count % 500 == 0:  # 这里表示控制器执行一千规划器执行一次
        ...
        for point in local_frenet_path_opt:
            ...
            debug.draw_point(carla.Location(point[0], point[1], 2),
                             size=0.05, color=carla.Color(0, 255, 0), life_time=1)
        ...
        # 提取障碍物的位置信息
        if len(possible_obs) != 0 and possible_obs[0][1] <= 30 and possible_obs[-1][
            1] > 10:  # 后面可以设计一下检测到可能障碍物后将最大速度降到30，离开后再恢复到道路要求的速度
            obs_xy = []
        ...
        # 注意重新实例化控制器的位置，不能放错了
        Controller = Vehicle_control(ego_vehicle=model3_actor, vehicle_para=vehicle_para,
                                     pathway=local_frenet_path_opt)  # 依据当前局部路径实例化控制器
